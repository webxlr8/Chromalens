...
import COLOR_NAMES from './color-data.js';
export interface RGB { r: number; g: number; b: number; }
export interface HSL { h: number; s: number; l: number; }
export interface Lab { L: number; a: number; b: number; }
export interface WeightedRGB extends RGB { count?: number; }
export interface Cluster extends RGB { count: number; }
interface LabPoint extends Lab { count: number; }
interface ColorPoint { hex: string; name: string; rgb: RGB; lab: Lab; }
...
export function getHarmonies(hex: string): { complementary: string[]; analogous: string[]; triadic: string[]; 'split-complementary': string[]; tetradic: string[]; square: string[]; monochromatic: string[] } {
...
export function kMeansClustering(pixels: Array<RGB | WeightedRGB | [number, number, number]>, k: number, maxIterations = 20): Cluster[] {
    if (pixels.length === 0) return [];

    // Standardize input to {r,g,b, count} then convert to LAB
    const rgbPoints: Array<{ r: number; g: number; b: number; count: number }> = pixels.map(p =>
        Array.isArray(p)
            ? { r: p[0], g: p[1], b: p[2], count: 1 }
            : { r: p.r, g: p.g, b: p.b, count: 'count' in p ? p.count ?? 1 : 1 }
    );

    // Convert to LAB for perceptually uniform clustering
    const points: LabPoint[] = rgbPoints.map(p => {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            const i0 = clusterIdx * 3;
            sums[i0] = (sums[i0] ?? 0) + p.L * p.count;
            sums[i0 + 1] = (sums[i0 + 1] ?? 0) + p.a * p.count;
            sums[i0 + 2] = (sums[i0 + 2] ?? 0) + p.b * p.count;
            counts[clusterIdx] = (counts[clusterIdx] ?? 0) + p.count;
        }
        ...
        for (let j = 0; j < k; j++) {
            if ((counts[j] ?? 0) > 0) {
                centroids[j] = {
                    L: (sums[j * 3] ?? 0) / (counts[j] ?? 1),
                    a: (sums[j * 3 + 1] ?? 0) / (counts[j] ?? 1),
                    b: (sums[j * 3 + 2] ?? 0) / (counts[j] ?? 1)
                };
            }
        }
    }
...
class VPNode {
    ...
    constructor(point: ColorPoint, left: VPNode | null = null, right: VPNode | null = null, radius = 0) {
        this.point = point;
        this.left = left;
        this.right = right;
        this.radius = radius;
...
class ColorSearchTree {
    ...
    buildTree(points: ColorPoint[]): VPNode | null {
    ...
    findNearest(targetLab: Lab): string {
        if (!this.root) return 'Unknown';

        const best = { node: this.root, dist: Infinity };
        this.search(this.root, targetLab, best);
        return best.node.point.name;
    }

    search(node: VPNode | null, target: Lab, best: { node: VPNode; dist: number }): void {
        if (!node) return;
...
const colorsList: ColorPoint[] = Object.entries(COLOR_NAMES).map(([hex, name]) => {
